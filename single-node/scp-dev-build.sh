apply-docker-image() {
  ...
  for vm in default ; do vagrant ssh $vm -- docker tag quay.io/rook/$name:$tag quay.io/rook/$name:dev; done
  #for vm in default ; do vagrant ssh $vm -- docker tag $imagename $imagename:dev; done
}
